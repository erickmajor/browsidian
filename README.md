...
- **Dataview support**: The app builds an internal metadata cache on vault load (frontmatter + inline tags + wikilinks) so `TABLE`, `LIST`, and `TASK` queries with `FROM #tag` and `FROM [[link]]` clauses return real results.
